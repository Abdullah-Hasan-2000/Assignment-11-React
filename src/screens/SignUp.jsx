                    ...
                    <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form className="space-y-4 md:space-y-6" onSubmit={handleSignUp}>
                                ...
                                <button type="submit" className="cursor-pointer w-full text-white bg-rose-600 hover:bg-rose-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-rose-600 dark:hover:bg-rose-700 dark:focus:ring-rose-800" disabled={loading}>{loading ? 'Signing up...' : 'Sign up'}</button>
                                {error && <p className="text-red-500 text-sm">{error}</p>}
                                {user && <p className="text-green-500 text-sm">Signup successful!</p>}
                                <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                                    Already have an account? <span onClick={()=> navigate('/login')} href="#" className="font-medium text-rose-600 hover:underline dark:text-rose-500 cursor-pointer">Sign in</span>
                                </p>
                            </form>
                        </div>
                    </div>
